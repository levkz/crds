...
| `internal/cli/` | 22 | Kong command wiring |
| `internal/ai/` | 49 | AI agent: providers, config, prompts, parsing |
...
Total: **406 test functions**, all passing.
...
| `internal/cli/` | Kong commands: quiz, stats, deck (list/import/export/delete/search/edit), term (add/rm/edit), tag (add/rm/list), state (reserve/revert/sync), profile (export/import), ai (interpret/fill/add, incl. `--full`/`--minimal`/`--msg`) | 22 |
| `internal/ai/` | Agent: 7 provider presets (pollinations, ollama, openai, gemini, openrouter, groq, nvidia), OpenAI-compatible client, prompts (minimal/full interpret, fill, `--msg` passthrough), YAML parsing, interpret/fill agents | 49 |
