...
// agent: unstructured free text -> structured YAML entries.
func InterpretMessages(raw string, lc LanguageContext) (string, string) {
...
// (possibly partial) YAML entries -> completed entries.
func FillMessages(entries []model.Entry, dc DeckContext) (string, string) {
...
- Add 2-3 example sentences in the source language, each with a translation in the target language. Examples must be natural and correct.
...
Term and translation conventions (use CRDS variant syntax):
